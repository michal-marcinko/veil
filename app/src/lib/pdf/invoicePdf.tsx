// ---------------------------------------------------------------------------
// Pre-payment invoice PDF (heavy variant).
//
// Two render modes (selected by the `auditMode` prop):
//   - false (DEFAULT): paper-grade invoice. Line items, total, parties.
//     NO on-chain refs, no payer wallet, no tx sigs. Same disclosure
//     surface as a paper invoice — the right thing to email customers.
//   - true: same body + AuditFooter at the bottom. Surfaces invoice PDA,
//     network, slot, status, and the verifier QR.
//
// Visually mirrors the existing `app/src/lib/invoicePdf.tsx` — same brand
// palette, same column structure, same Helvetica + Courier rhythm — so an
// auditor receiving both formats sees one consistent document family.
// ---------------------------------------------------------------------------
...
// Brand palette — keep in sync with tailwind.config.ts and the existing
// `lib/invoicePdf.tsx` template.
const PAPER = "#f8f4e9";
const INK = "#1c1712";
const MUTED = "#736b57";
const DIM = "#a59c84";
...
const styles = StyleSheet.create({
  ...
  invoiceId: { fontSize: 11, fontFamily: "Courier", color: INK },
  partyRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginBottom: 28,
...
export interface InvoicePdfDocumentProps {
  ...
  invoicePda: string;
  /**
   * When true, append an AuditFooter with on-chain refs + QR to the
   * verifier. Default false — the user has to opt in via the "Include
   * audit details" checkbox before the PDF is generated.
   */
  auditMode?: boolean;
  /**
   * Required when `auditMode` is true. Used to build the verify URL's
   * capability token. Plumbed through from the on-chain Invoice account.
   */
  metadataHash?: Uint8Array;
  /** Network the on-chain refs belong to. */
  network?: "devnet" | "mainnet";
  /** On-chain creation slot, surfaced in the audit footer. */
  slot?: number | null;
  /** ISO timestamp of when the on-chain invoice was created (createdAt seconds × 1000). */
  createdAtIso?: string;
  /** "Pending" | "Paid" — surfaced in the audit footer. */
  chainStatus?: "Pending" | "Paid";
  /** Optional signMessage signature over canonical audit-fact bytes. */
...
export function InvoicePdfDocument(props: InvoicePdfDocumentProps) {
  ...
  // Audit footer prep — only computed when auditMode is on.
  const verifyUrl =
    auditMode && metadataHash
      ? buildVerifyUrl({ invoicePda, metadataHash })
      : null;
  const facts: AuditFooterFact[] = auditMode
    ? [
        ...
        ...(createdAtIso ? [{ label: "Created at", value: createdAtIso }] : []),
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        {/* Audit footer — only when explicitly opted in. */}
        {auditMode && verifyUrl ? (
          <AuditFooter
            verifyUrl={verifyUrl}
            facts={facts}
